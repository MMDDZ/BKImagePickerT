...
-(void)setDirection:(BKIPTransitionAnimaterDirection)direction
{
    if ([self isKindOfClass:[BKImagePickerNavigationController class]]) {
        [self changeValue:@(direction) forProperty:@"_private_direction"];
    }
}

-(BKIPTransitionAnimaterDirection)direction
{
    if ([self isKindOfClass:[BKImagePickerNavigationController class]]) {
